...
public class UserSharedPreference {
    ...
    public void removeJwtToken() {
        ...
        HttpDog.getInstance().updateCommonHeader("Authorization", "Bearer null");
    }
    ...
    public void setUser(UserEntity user) {
        this.putUserToCache(user);

        this.putUserToSharedPreference(user);
    }

    private void putUserToCache(UserEntity user) {
        mACache.put(CACHE_USER_KEY, user, CACHE_SECONDS);
    }

    private void putUserToSharedPreference(UserEntity user) {
        mEditor.putString(CACHE_USER_KEY, JSON.toJSONString(user));
        mEditor.commit();
    }
    ...
    public UserEntity getUser() {
        UserEntity user = this.getUserFromCache();

        if (null == user) {
            user = this.getUserFromSharedPreference();
    ...
    private UserEntity getUserFromCache() {
        Object user = mACache.getAsObject(CACHE_USER_KEY);
        if (null == user) {
            return null;
        }
    ...
    private UserEntity getUserFromSharedPreference() {
        if (!mSharedPreferences.contains(CACHE_USER_KEY)) {
            return null;
        }
        String user = mSharedPreferences.getString(CACHE_USER_KEY, null);
        if (null == user) {
            return null;
        }
